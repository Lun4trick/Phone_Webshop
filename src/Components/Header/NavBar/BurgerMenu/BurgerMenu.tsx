...
export const BurgerMenu: React.FC<Props> = ({ isMenuOpen }) => {
  ...
  return (
    ...
    >
      <ul className="flex flex-col text-slate-100 text-xs text-center tablet:hidden items-center mb-8">
        {NavItems.map(item => (
          <NavLink 
            key={item.path}
            to={item.path}
            className="flex font-mont font-semibold text-[#F1F2F9] items-center border-b-2 border-transparent hover:border-white transition-all duration-300 pt-8"
          >
            <button
              className='uppercase'
              type='button'
      ...
      <div className="tablet:hidden w-full border-t border-gray-600 h-[64px]">
